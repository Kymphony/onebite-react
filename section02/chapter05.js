...
원시타입: 값 자체로써 변수에 저장되고 복사된다. (메모리에 그대로 값 할당)
객테차입: 값 자체가 아닌 값에 접근할 수 있는 주소값(참조값)을 통해 변수에 저장되고 복사된다.
*/
...
// 즉, 원시타입 변수에 한번 할당된 값은 메모리에 그대로 남아 변하지 않기 때문에 원시타입을 "불변값" 이라고도 한다.
